...
/* Vanzo:zhongjunyu on: Mon, 09 Mar 2015 11:37:28 +0800
...
/* Vanzo:zhongjunyu on: Mon, 09 Mar 2015 11:37:34 +0800
 * add gy v GestureShot
 */
    private ICameraContext mContext;
// End of Vanzo:zhongjunyu
...
/* Vanzo:zhongjunyu on: Mon, 09 Mar 2015 11:37:38 +0800
 * add gy v GestureShot
 */
        mContext = context;
// End of Vanzo:zhongjunyu
...
/* Vanzo:zhongjunyu on: Mon, 09 Mar 2015 11:37:50 +0800
 * add gy v GestureShot
 */
                if (FeatureSwitcher.isSupportGYVGestureShot()) {
                    updateCameraDevice();
                    if (mICameraDevice != null) {
                        mICameraDevice.getParameters().setPreviewFormat(
                                android.graphics.ImageFormat.NV21);
                        mICameraDevice.applyParameters();
                        int bufferSize = getBufferSize();
                        byte[] cacheBuffer = new byte[bufferSize];
                        mICameraDevice.addCallbackBuffer(cacheBuffer);
                        mICameraDevice
                            .setPreviewCallbackWithBuffer(mVanzoGestureCallback);
                    }
                }
// End of Vanzo:zhongjunyu
            }
...
/* Vanzo:zhongjunyu on: Mon, 09 Mar 2015 11:38:05 +0800
 * add gy v GestureShot
 ...
 */
        if (FeatureSwitcher.isSupportGYVGestureShot()) {
            ...
            byte[] cacheBuffer = new byte[bufferSize];
            mICameraDevice.addCallbackBuffer(cacheBuffer);
            mICameraDevice.setPreviewCallbackWithBuffer(mVanzoGestureCallback);
            Log.w(TAG, "start gy GuestureDetection");
        } else {
            mICameraDevice.setGestureCallback(mGestureListener);
            mICameraDevice.startGestureDetection();
        }
...
/* Vanzo:zhongjunyu on: Mon, 09 Mar 2015 11:38:21 +0800
 * add gy v GestureShot
        if (mICameraDevice != null) {
 ...
 */
        if (FeatureSwitcher.isSupportGYVGestureShot()) {
            ...
            mICameraDevice.applyParameters();
            mICameraDevice.setPreviewCallbackWithBuffer(null);
        } else {
            if (mICameraDevice != null) {
                mICameraDevice.stopGestureDetection();
                mICameraDevice.setGestureCallback(null);
        ...
        public void onGesture() {
            ...
            if (!mCameraClosed && (mICameraAppUi.getViewState() != ViewState.VIEW_STATE_SETTING)) {
                ...
                if (mISelfTimeManager.isSelfTimerEnabled()) {
                    ...
                    // stopGuestureDetection();
/* Vanzo:zhongjunyu on: Mon, 09 Mar 2015 11:38:41 +0800
 * add gy v GestureShot
 */
                    if (FeatureSwitcher.isSupportGYVGestureShot()) {
                        mICameraDevice.setPreviewCallbackWithBuffer(null);
...
/* Vanzo:zhongjunyu on: Mon, 09 Mar 2015 11:38:48 +0800
 * add gy v GestureShot
 */
    protected final VanzoActorGestureCallback mVanzoGestureCallback = new VanzoActorGestureCallback();

    private class VanzoActorGestureCallback implements PreviewCallback {

        public void onPreviewFrame(byte[] rawData,
            ...
            int yuvDegree = (((CameraActivity) mContext.getActivity())
                    .getOrietation() + 90) % 360;
            if (mICameraDevice.getCameraId() == CameraInfo.CAMERA_FACING_BACK) {
                yuvDegree = (270 - ((CameraActivity) mContext.getActivity())
                        .getOrietation()) % 360;
            }

            if (com.gangyun.camera.LibDetectFeature.DetectVGesture(w, h,
                        rawData, mICameraDevice.getCameraId(), yuvDegree, 0)) {
                if (mICameraDevice.getCameraId() == CameraInfo.CAMERA_FACING_BACK) {
                    if (((CameraActivity) mContext.getActivity())
                            .getFrameView().faceExists()) {
                        mGestureListener.onGesture();
                    }
                } else {
                    mGestureListener.onGesture();
                }
            ...
            mICameraDevice.addCallbackBuffer(rawData);
        }
    }
    ...
    private int getBufferSize() {
        ...
        int size = mICameraDevice.getParameters().getPreviewSize().width
            * mICameraDevice.getParameters().getPreviewSize().height;
        size = size
            * ImageFormat.getBitsPerPixel(mICameraDevice.getParameters()
                    .getPreviewFormat()) / 8;
        return size;
    }
// End of Vanzo:zhongjunyu
