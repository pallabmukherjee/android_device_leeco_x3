...
import com.mediatek.galleryfeature.xmp.XmpOperator;
...
public class StereoCameraMode extends PhotoMode {
  ...
  //
    ...
    private void setRefocusSettingRules(ICameraContext cameraContext) {
        ...
        mISettingCtrl.addRule(SettingConstants.KEY_REFOCUS, SettingConstants.KEY_PICTURE_RATIO, previewSizeRule);
    }
    ...
    private class MaskCallback implements StereoMaskCallback {
        public void onCapture(byte[] maskData) {
            ...
            showQualityStatus((int)mMaskAndConfigData[GEO_INFO], (int)mMaskAndConfigData[PHO_INFO]);
            notifyMergeData();
        }
    }
        ...
        public void execute() {
            ...
            if ("on".equals(currentValue)) {
                ...
                if (mPictureRatioSetting.isEnable()) {
                    ...
                    SettingUtils.setPreviewSize(mCameraContext.getActivity(), parameters, overrideValue);
                }
    ...
    private void showQualityStatus(int geoFlag, int photoFlag) {
        ...
        new AlertDialog.Builder(mActivity).setCancelable(false)
                ...
                .setMessage(msg).setNeutralButton(R.string.dialog_ok, buttonListener).show();
    }
    ...
    private boolean isDebugOpened() {
        boolean enable = SystemProperties.getInt("debug.STEREO.enable_verify", 0) == 1 ? true : false;
