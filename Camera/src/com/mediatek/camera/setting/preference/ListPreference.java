...
/* Vanzo:hanshengpeng on: Tue, 10 Feb 2015 15:45:04 +0800
 */
...
/* Vanzo:hanshengpeng on: Tue, 10 Feb 2015 15:37:58 +0800
 * add feature, camera picture size string 3M -> 3.2M
                entries.add(mOriginalEntries[i]);
 */
                if (FeatureSwitcher.isSupportPictureSize3mto3m2()) {
                    if (SettingConstants.KEY_PICTURE_SIZE.equals(mKey)
                            &&("2048x1536".equals(mOriginalEntryValues[i])||
                                "2048x1152".equals(mOriginalEntryValues[i]))) {
                        String str = mOriginalEntries[i].toString();
                        if (str != null && str.contains("300")) {
                            entries.add(str.replace("300", "320"));
                        } else if (str != null) {
                            entries.add(str.replace("3", "3.2"));
                        } else {
                            entries.add(mOriginalEntries[i]);
                        }
                    } else {
