...
/* Vanzo:libing on: Mon, 26 Jan 2015 17:26:46 +0800
 */
import java.io.File;
...
import com.mediatek.gallery3d.util.GestureStateUtils;
...
/* Vanzo:libing on: Thu, 10 Apr 2014 18:07:02 +0800
 * add for gesture
 */
        SensorEventListener,
// End of Vanzo:libing
...
/* Vanzo:tanglei on: Sat, 07 Mar 2015 11:15:56 +0800
 */
    private LayoutInflater mInflater;
    private ListPreference mFaceBeautyPref;
    private InLineSettingItem mFaceBeautyView;
...
/* Vanzo:yucheng on: Thu, 16 Apr 2015 20:17:18 +0800
 * Optiomiztion for camera, If camera is opened,
 * the action to close flashlight must be sent immediately
 */
        getApplicationContext().sendBroadcast(new Intent("com.android.systemui.qs.tiles.close.flashlight"));
// End of Vanzo: yucheng
/* Vanzo:libing on: Thu, 10 Apr 2014 18:08:45 +0800
 * add for gesture
 */
        if (isSupportPsensorGesture()) {
            android.util.Log.d(TAG, "Camera initPsensorManager");
            initPsensorManager();
        }
// End of Vanzo:libing
...
/* Vanzo:libing on: Thu, 10 Apr 2014 18:08:30 +0800
 * init for gesture
 */
    private static final String GESTURE_SWITCH_FLAG = "persist.sys.gesture_switch";
    private static final String GESTURE_CAMERA_FLAG = "persist.sys.gesture_camera";
    // 1.first psensor value large than the GESTURE_CAMERA_MAX_DISTANCE
    private static final int GESTURE_CAMERA_MAX_DISTANCE = 350;
    // 2.second psensor value small than the GESTURE_CAMERA_MIN_DISTANCE
    private static final int GESTURE_CAMERA_MIN_DISTANCE = 150;
    ...
    public static boolean mIsCameraAtTop;
...
/* Vanzo:libing on: Thu, 10 Apr 2014 18:13:39 +0800
 * add for gesture
 */
        if (isSupportPsensorGesture()) {
            android.util.Log.d(TAG, "Camera onResume isGestureOpen = " + isGestureOpen());
            if (isGestureOpen()) {
                resumePsensor();
            }
        } else if (isSupportNewSensorGesture()) {//new sensor gesture
            Log.d(TAG, "Camera ===== onResume new sensor sIsEnterCameraPreview = " + PhotoView.sIsEnterCameraPreview);
            resumeNewGestureSensor();
        }
// End of Vanzo:libing
/* Vanzo:yucheng on: Thu, 16 Apr 2015 20:17:18 +0800
 * Optiomiztion for camera, If camera is resumed,
 * must make sure flashlight has been closed
 */
        getApplicationContext().sendBroadcast(new Intent("com.android.systemui.qs.tiles.close.flashlight"));
// End of Vanzo: yucheng
...
/* Vanzo:libing on: Thu, 10 Apr 2014 18:11:28 +0800
 * add for gesture
 */
    public boolean isGestureOpen() {
        mGestureEnabled = SystemProperties.getInt(GESTURE_SWITCH_FLAG , 0) == 1 ? true : false;
        mCameraEnabled = SystemProperties.getInt(GESTURE_CAMERA_FLAG , 0) == 1 ? true : false;
        return mGestureEnabled && mCameraEnabled;
    }
    ...
    public Runnable myRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if (file.exists()) {
                ...
                if (doTakePic()) {
                    Log.d(TAG, "run() mGestureEnabled = " + mGestureEnabled + " mCameraEnabled = " + mCameraEnabled);
                    if (mGestureEnabled && mCameraEnabled && (mCameraState != STATE_SNAPSHOT_IN_PROGRESS)) {
                        if (mPrePsensor > GESTURE_CAMERA_MAX_DISTANCE) {
                            mHadGesture = true;
                        } else if (mHadGesture && mPrePsensor < GESTURE_CAMERA_MIN_DISTANCE) {
                            mHadGesture = false;
                            OnShutterButtonListener listener = mCameraActor.getPhotoShutterButtonListener();
                            if (listener != null) {
                                listener.onShutterButtonClick(null);
                            }
                        }
                    }
                }
    ...
    private boolean doTakePic() {
        if (isTopActivity() && PhotoView.sIsEnterCameraPreview) {
            return true;
        } else {
            return false;
        }
    ...
    private boolean isTopActivity() {
        ...
        String myClassName = CameraActivity.this.getComponentName().getClassName();
    ...
    private void stopPsensor() {
        android.util.Log.d(TAG, "Camera stopPsensor");
        if (mSensorManager != null) {
            mSensorManager.unregisterListener(this);
        }
        if (myHandler != null) {
            myHandler.removeCallbacks(myRunnable);
            Log.v(TAG, "Camera stopPsensor removeCallbacks myHandler = " + myHandler);
        }
    }

    private void resumePsensor() {
        android.util.Log.d(TAG, "Camera resumePsensor ");
    ...
    private boolean isSupportPsensorGesture() {
        return FeatureOption.VANZO_FEATURE_GALLERY_CAMERA_GESTURE;
    }

    private boolean isSupportNewSensorGesture() {
        return FeatureOption.VANZO_FEATURE_GALLERY_CAMERA_NEW_GESTURE;
    }
// End of Vanzo:libing
    @Override
...
/* Vanzo:tanglei on: Mon, 09 Mar 2015 17:04:45 +0800
 * move facebeauty settings to modepick
 */
        if (FeatureOption.VANZO_FEATURE_CAMERA_MOVE_FACEBEAUTY_SETTINGS_TO_MODEPICK) {
            collapseFaceBeautyView();
        }
// End of Vanzo:tanglei
        ...
        Util.exitCameraPQMode();
        mOtherDeviceConectedManager.onPause();
/* Vanzo:libing on: Fri, 09 May 2014 18:13:07 +0800
 * new sensor gesture
 */
        if (isSupportNewSensorGesture()) {
            Log.d(TAG, "Camera ===== new sensor onPause dissableGestureSensor");
            pauseNewGestureSensor();
        }
// End of Vanzo:libing
    }
...
/* Vanzo:libing on: Thu, 10 Apr 2014 18:15:53 +0800
 * add for gesture
 */
        if (isSupportPsensorGesture()) {
            if (myHandler != null) {
                myHandler.removeCallbacks(myRunnable);
                Log.v(TAG, "Camera onDestroy() removeCallbacks myHandler = " + myHandler);
            }
        }
// End of Vanzo:libing
    }

/* Vanzo:libing on: Fri, 09 May 2014 18:26:47 +0800
 * new sensor gesture
 */
    private boolean mGalleryEnabled;
    private static final String GESTURE_GALLERY_FLAG = "persist.sys.gesture_gallery";
    // new sensor gesture run hander delay time 350 is 0.35s
    private static final int NEW_GESTURE_RUN_POST_DELAY_TIME = 350;
    // 1 down--->up // 2 up--->down // 3 left--->right // 4 right--->left
    private static final int NEW_GESTURE_SWITCH_FLAG_DOWN_TO_UP = 1;
    private static final int NEW_GESTURE_SWITCH_FLAG_UP_TO_DOWN = 2;
    private static final int NEW_GESTURE_SWITCH_FLAG_LEFT_TO_RIGHT = 3;
    private static final int NEW_GESTURE_SWITCH_FLAG_RIGHT_TO_LEFT = 4;

    private Handler newGestureHandler;
    private int mGestureSwitchState;
    public Runnable newGestureRunnable = new Runnable() {
        @Override
            public void run() {
                if (mCameraState != STATE_SNAPSHOT_IN_PROGRESS && doTakePic()) {
                    mGestureSwitchState = GestureStateUtils.getGestureSwitchValue();
                    Log.v(TAG, "new sensor: Camera  Runnable mGestureSwitchState ===== " + mGestureSwitchState);
                    if (mGestureSwitchState == NEW_GESTURE_SWITCH_FLAG_DOWN_TO_UP) {
                        Log.v(TAG, "new sensor: Camera  Runnable  ===== down--->up ");
                    } else if (mGestureSwitchState == NEW_GESTURE_SWITCH_FLAG_UP_TO_DOWN) {
                        Log.v(TAG, "new sensor: Camera  Runnable  ===== up --->down takePicture");
                        handlePictureForNewGesture();
                    } else if (mGestureSwitchState == NEW_GESTURE_SWITCH_FLAG_LEFT_TO_RIGHT) {
                        Log.v(TAG, "new sensor: Camera  Runnable  ===== left--->right takePicture");
                        handlePictureForNewGesture();
                    } else if (mGestureSwitchState == NEW_GESTURE_SWITCH_FLAG_RIGHT_TO_LEFT) {
                        Log.v(TAG, "new sensor: Camera  Runnable  ===== right--->left ");
                    }
                }
                newGestureHandler.postDelayed(newGestureRunnable, NEW_GESTURE_RUN_POST_DELAY_TIME);
            }
    };

    private void handlePictureForNewGesture() {
        OnShutterButtonListener listener = mCameraActor.getPhotoShutterButtonListener();
        if (listener != null) {
            Log.v(TAG, "new sensor: Camera  takePicture ===== ");
            listener.onShutterButtonClick(null);
            Log.v(TAG, "new sensor: Camera  takePicture ===== clearGestureSwitch");
            GestureStateUtils.clearGestureSwitch();
        }
    }

    private void resumeNewGestureSensor() {
        mGestureEnabled = SystemProperties.getInt(GESTURE_SWITCH_FLAG , 0) == 1 ? true : false;
        mCameraEnabled = SystemProperties.getInt(GESTURE_CAMERA_FLAG , 0) == 1 ? true : false;
        mGalleryEnabled = SystemProperties.getInt(GESTURE_GALLERY_FLAG , 0) == 1 ? true : false;

        Log.d(TAG, "new sensor: Camera ===== onResume isTopActivity = " + isTopActivity());
        if (PhotoView.sIsEnterCameraPreview && isTopActivity()) {//goto Camera preview, set sensor to Switch mode
            if (mGestureEnabled && mCameraEnabled) {
                Log.d(TAG, "new sensor: Camera ===== onResume enableGestureSwitchMode ");
                GestureStateUtils.enableGestureSwitchMode();
                GestureStateUtils.clearGestureSwitch();
                Log.d(TAG, "new sensor: Camera ===== onResume post Runnable ");
                newGestureHandler = new Handler();
                newGestureHandler.post(newGestureRunnable);
            }
        } else {//from CameraPreview to view photo, set TP mode
            if (mGestureEnabled && mGalleryEnabled) {
                GestureStateUtils.enableGestureTPMode();
                Log.d(TAG, "new sensor: Camera ===== onResume enableGestureTPMode ");
            }
        }
    }

    private void pauseNewGestureSensor() {
        GestureStateUtils.dissableGestureSensor();
        Log.d(TAG, "new sensor: Camera ===== pauseGestureSensor removeGestureHandler");
        removeNewGestureHandler();
    }

    private void removeNewGestureHandler() {
        if (newGestureHandler != null) {
            newGestureHandler.removeCallbacks(newGestureRunnable);
            Log.v(TAG, "new sensor: Camera removeGestureHandler() newGestureHandler = " + newGestureHandler);
        }
    }
// End of Vanzo:libing
    @Override
...
/* Vanzo:tanglei on: Mon, 09 Mar 2015 16:51:37 +0800
 * move facebeauty settings to modepick
 */
        if (FeatureOption.VANZO_FEATURE_CAMERA_MOVE_FACEBEAUTY_SETTINGS_TO_MODEPICK) {
            if (collapseFaceBeautyView()) {
                return;
            }
        }
// End of Vanzo:tanglei
...
/* Vanzo:tanglei on: Mon, 09 Mar 2015 17:02:13 +0800
 * move facebeauty settings to modepick
 */
            if (FeatureOption.VANZO_FEATURE_CAMERA_MOVE_FACEBEAUTY_SETTINGS_TO_MODEPICK) {
                collapseFaceBeautyView();
...
/* Vanzo:tanglei on: Mon, 09 Mar 2015 17:02:29 +0800
 * move facebeauty settings to modepick
 */
            if (FeatureOption.VANZO_FEATURE_CAMERA_MOVE_FACEBEAUTY_SETTINGS_TO_MODEPICK) {
                collapseFaceBeautyView();
...
/* Vanzo:tanglei on: Mon, 09 Mar 2015 17:03:04 +0800
 * move facebeauty settings to modepick
 */
        if (FeatureOption.VANZO_FEATURE_CAMERA_MOVE_FACEBEAUTY_SETTINGS_TO_MODEPICK) {
            collapseFaceBeautyView();
...
// End of Vanzo:tanglei
    ...
    private void restoreWhenCameraOpenFailed() {
        ...
        if (mNeedRestoreIfOpenFailed) {
            ...
            callResumablePause();
            mCameraAppUi.collapseViewManager(true);
/* Vanzo:tanglei on: Mon, 09 Mar 2015 17:03:19 +0800
 * move facebeauty settings to modepick
 */
            if (FeatureOption.VANZO_FEATURE_CAMERA_MOVE_FACEBEAUTY_SETTINGS_TO_MODEPICK) {
                collapseFaceBeautyView();
...
/* Vanzo:tanglei on: Sat, 07 Mar 2015 11:14:29 +0800
 * move facebeauty settings to modepick
 */
    public boolean collapseFaceBeautyView() {
        Log.d("tanglei", "collapseFaceBeautyView");
        if (mFaceBeautyView != null) {
            return mFaceBeautyView.collapseChild();
        }
        return false;
    }
// End of Vanzo:tanglei
        ...
        public void onModeChanged(int newMode) {
            ...
            if (mCameraActor.getMode() != newMode) {
                ...
                case ModePicker.MODE_FACE_BEAUTY:
                    mCameraActor = new PhotoActor(CameraActivity.this, mModuleManager, newMode);
/* Vanzo:tanglei on: Sat, 07 Mar 2015 11:13:06 +0800
 * move facebeauty settings to modepick
 */
                    if (FeatureOption.VANZO_FEATURE_CAMERA_MOVE_FACEBEAUTY_SETTINGS_TO_MODEPICK) {
                        if (mFaceBeautyView == null) {
                            mFaceBeautyPref = getListPreference(SettingConstants.KEY_FACE_BEAUTY_PROPERTIES);
                            int viewLayoutId = R.layout.in_line_setting_virtual;
                            mInflater = LayoutInflater.from(CameraActivity.this);
                            ViewGroup appRoot = (ViewGroup) findViewById(R.id.camera_app_root);
                            mFaceBeautyView = (InLineSettingItem) getLayoutInflater().inflate(viewLayoutId, appRoot, false);
                            mFaceBeautyView.initialize(mFaceBeautyPref);
                        }
                        mFaceBeautyView.expendChild();
                    }
// End of Vanzo:tanglei
                    break;
    ...
    private void releaseCameraActor(int oldMode, int newMode) {
        ...
        if (shouldHideSetting) {
            mCameraAppUi.collapseViewManager(true);
/* Vanzo:tanglei on: Mon, 09 Mar 2015 17:03:41 +0800
 * move facebeauty settings to modepick
 */
            if (FeatureOption.VANZO_FEATURE_CAMERA_MOVE_FACEBEAUTY_SETTINGS_TO_MODEPICK) {
                collapseFaceBeautyView();
...
/* Vanzo:tanglei on: Mon, 09 Mar 2015 17:03:58 +0800
 * move facebeauty settings to modepick
 */
                    if (FeatureOption.VANZO_FEATURE_CAMERA_MOVE_FACEBEAUTY_SETTINGS_TO_MODEPICK) {
                        collapseFaceBeautyView();
...
/* Vanzo:hanshengpeng on: Thu, 22 Jan 2015 17:50:56 +0800
...
/* Vanzo:tanglei on: Fri, 14 Aug 2015 21:19:22 +0800
 * default face beauty off
 */
                    mISettingCtrl.onSettingChanged(SettingConstants.KEY_MULTI_FACE_BEAUTY, "Off");
// End of Vanzo:tanglei
    ...
    private static final String EXTRA_RESOLUTION_LIMIT = "mediatek.intent.extra.EXTRA_RESOLUTION_LIMIT";
    ...
    private boolean isRefocusSwitchVideo(int lastMode, int newMode) {
        Log.i(TAG, "isRefocusSwitchVideo lastMode = " + lastMode + ", newMode = " + newMode);
        return lastMode == ModePicker.MODE_STEREO_CAMERA && newMode == ModePicker.MODE_VIDEO;
    }
    
    private boolean isFastAfEnabled() {
        Log.i(TAG, "isFastAfEnabled" + getListPreference(SettingConstants.KEY_FAST_AF));
        return getListPreference(SettingConstants.KEY_FAST_AF) != null && "off".equals(getListPreference(SettingConstants.KEY_FAST_AF).getValue());
    }
    ...
    private void doPIPModeChanged(int cameraId) {
        ...
        mCameraAppUi.collapseViewManager(true);
/* Vanzo:tanglei on: Mon, 09 Mar 2015 17:04:28 +0800
 * move facebeauty settings to modepick
 */
        if (FeatureOption.VANZO_FEATURE_CAMERA_MOVE_FACEBEAUTY_SETTINGS_TO_MODEPICK) {
            collapseFaceBeautyView();
...
/* Vanzo:hanshengpeng on: Mon, 26 Jan 2015 20:40:53 +0800
 * add feature, time stamp
 */
    public boolean getEnableTimeStamp() {
        return mISettingCtrl.getEnableTimeStamp();
    }
// End of Vanzo:hanshengpeng
