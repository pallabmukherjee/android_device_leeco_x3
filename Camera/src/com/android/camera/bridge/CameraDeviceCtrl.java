...
public class CameraDeviceCtrl implements SurfaceHolder.Callback {
    ...
    private int getDelayTime() {
        ...
        if ((ModePicker.MODE_FACE_BEAUTY == mCameraActivity.getPrevMode() || "on"
            ...
            delayTime = 30;
        } else if (ModePicker.MODE_VIDEO == mCameraActivity.getCurrentMode()) {
            if (Integer.toString(CamcorderProfileEx.QUALITY_HIGH).equals(
                    mISettingCtrl.getSetting(SettingConstants.KEY_VIDEO_QUALITY).getValue())) {
                Log.d(TAG, "[getDelayTime] QUALITY_HIGH");
                delayTime = 30;
            }
            if (Integer.toString(CamcorderProfileEx.QUALITY_FINE).equals(
                    mISettingCtrl.getSetting(SettingConstants.KEY_VIDEO_QUALITY).getValue())
                    || Integer.toString(CamcorderProfileEx.QUALITY_FINE_4K2K)
                            .equals(mISettingCtrl.getSetting(SettingConstants.KEY_VIDEO_QUALITY)
                                    .getValue())) {
                Log.d(TAG, "[getDelayTime] QUALITY_FINE/QUALITY_FINE_4K2K");
                delayTime = 30;
            }
        }
