...
#define GET_DIFF_MARGIN_NUM(_rIdx_,_wIdx_)  ((((MINT32)_wIdx_)-_rIdx_)>=0)?(ISP_TUNING_QUEUE_NUM-_wIdx_+_rIdx_):(_rIdx_-_wIdx_)
...
)
{
  ...
  //  MINT32 queMarginNum = 0;
  //  static MINT32 conDiffNum[ISP_DRV_P2_CQ_NUM]={ 0, 0, 0};
  ...
  */  queMarginNum = GET_DIFF_MARGIN_NUM(getQueIdx, mTuningQueIdx[p2Cq].curWriteIdx);
    if(queMarginNum < ISP_TUNING_MARGIN_NUM) {
        conDiffNum[p2Cq] ++;
        if((conDiffNum[p2Cq] >= ISP_TUNING_CONTIN_TRIG_MAR_NUM) && (conDiffNum[p2Cq]%ISP_TUNING_CONTIN_TRIG_MAR_NUM == 0)) {
            LOG_WRN("[Warning]TuningQue is running low,p2Cq(%d),getQueIdx(%d),curWriteIdx(%d),queMarginNum(%d)",p2Cq,getQueIdx,mTuningQueIdx[p2Cq].curWriteIdx,queMarginNum);
        }
    } else {
        conDiffNum[p2Cq] = 0;
    }

    LOG_DBG("-,getQueIdx(%d),curWriteIdx(%d),queMarginNum(%d)",getQueIdx,mTuningQueIdx[p2Cq].curWriteIdx,queMarginNum);
    */
    return ret;
}
