...
{
    //
    MY_LOGD("+ (CamMode, DbgBuf, DbgSize, ExifBuf, pDbg_0, pDbg_1) = (%d, %p, %d, %p, %p, %p)",
            u4CamMode,  puDbgBuf, u4DbgSize, puExifBuf, pDbg_0, pDbg_1);

    bool ret = true;
    uint32_t const u4ThumbSize  = 0;
    uint32_t u4App1HeaderSize   = 0;
    uint32_t u4AppnHeaderSize_0 = 0;
    uint32_t u4AppnHeaderSize_1 = 0;
    uint32_t u4AppnStereoSize   = 0;
    uint32_t exifHeaderSize     = 0;
    CamExif rCamExif;
    CamExifParam rExifParam;
    CamDbgParam rDbgParam;
    ...
    // the bitstream doesn't rotated.
    rExifParam.u4Orientation    = mShotParam.mu4Transform;
    rExifParam.u4ZoomRatio      = mShotParam.mu4ZoomRatio;
    rExifParam.u4Facing = NSCam::IHalSensorList::get()->queryFacingDirection(getOpenId());
    //
    //! CamDbgParam (for camMode, shotMode)
    ...
    if ( pDbg_0 != NULL )
    {
        ...
        rCamExif.appendDebugExif(pdest, &u4AppnHeaderSize_0);
        pdest += u4AppnHeaderSize_0;
    }
    //
    NSCam::IHalSensor* pSensorHalObj = pHalSensorList->createSensor( ms8ShotName.string(), getOpenId());    // TODO: open id
    if( pSensorHalObj == NULL )
    {
       MY_LOGE("pSensorHalObj is NULL");
       ret = false;
    ...
    rCamExif.appendDebugExif(pdest, &u4AppnHeaderSize_1);   // TODO: sensor index
    pdest += u4AppnHeaderSize_1;
    //
    rCamExif.makeExifAppn(0xF, u4DbgSize, puDbgBuf, pdest, &u4AppnStereoSize);
    rCamExif.uninit();

    u4FinalExifSize = u4App1HeaderSize + u4AppnHeaderSize_0 + u4AppnHeaderSize_1 + u4AppnStereoSize;

    MY_LOGD("- (app1Size, appnSize_0, appnSize_1, stereoSize, exifSize) = (%d, %d, %d, %d, %d)",
              u4App1HeaderSize, u4AppnHeaderSize_0, u4AppnHeaderSize_1, u4AppnStereoSize, u4FinalExifSize);

    if ( STEREO_EXIF_SIZE < u4FinalExifSize )
    {
        MY_LOGE("u4FinalExifSize(%d) >= exif header size(%d)", u4FinalExifSize, STEREO_EXIF_SIZE);
        ret = false;
    }
