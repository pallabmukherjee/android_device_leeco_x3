...
)
{
    ...
    case eShotMode_NormalShot:
        ...
        if(pParamsMgr->getInt("ts-skin-enable") == 1){
           
		/*
          int32_t blurLevel = 100;
          int32_t whitenLevel = 100;
          int32_t colorId = 8;*/
			int enable = pParamsMgr->getInt("ts-skin-enable");
			int32_t blurLevel = pParamsMgr->getInt("ts-skin-blurLevel");
			int32_t whitenLevel = pParamsMgr->getInt("ts-skin-whitenLevel");
			int32_t colorId = pParamsMgr->getInt("ts-skin-colorId");
          rpShot = createInstance_TSFaceBeautyShot("FBShot", u4ShotMode, i4OpenId, blurLevel, whitenLevel, colorId);
       }
        else
        #endif
