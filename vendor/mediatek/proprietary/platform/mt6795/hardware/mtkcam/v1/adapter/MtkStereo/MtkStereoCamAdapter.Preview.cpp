...
onHandleStartPreview()
{
    ...
    targetWidth     = (recordHint) ? IMG_1080P_W : prvWidth;
    targetHeight    = (recordHint) ? IMG_1080P_H : prvHeight;
    MY_LOGD("TARGET:%dx%d, PRV:%dx%d = %d, VDO:%dx%d = %d, 1080P(%d)",
    ...
    if( mbZsdMode )
    {
        ...
        configInfo.rollbackBufNumPrv    = get_zsd_cap_stored_frame_cnt();
        cfgInfo_Main2.rollbackBufNum    = get_zsd_cap_stored_frame_cnt();
        cfgInfo_Main2.rollbackBufNumPrv = get_zsd_cap_stored_frame_cnt();
