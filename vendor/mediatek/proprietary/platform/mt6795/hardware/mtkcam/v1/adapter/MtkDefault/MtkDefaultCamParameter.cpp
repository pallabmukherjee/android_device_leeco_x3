...
setParameters()
{
    ...
    status_t ret = OK;
    //
       #if defined(VANZO_FEATURE_CAMERA_TS_FACEBEAUTY)
	   /*
	       mEnableskin = 1;
		   mBlurLevel = 100;
		   mWhitenLevel=100;
		   mColorId = 8;
		   */
           if (mpExtImgProcNode != NULL && mTsBeautyInit){
                mEnableskin = mpParamsMgr->getInt("ts-skin-enable");
                if(mEnableskin== 1 || mEnableskin ==0){
			
			mEnableskin = mpParamsMgr->getInt("ts-skin-enable");
			mBlurLevel = mpParamsMgr->getInt("ts-skin-blurLevel");
			mWhitenLevel = mpParamsMgr->getInt("ts-skin-whitenLevel");
			mColorId = mpParamsMgr->getInt("ts-skin-colorId");
			mpExtImgProcNode->setTSskinLevel(mEnableskin, mBlurLevel, mWhitenLevel, mColorId);
               }
           }
