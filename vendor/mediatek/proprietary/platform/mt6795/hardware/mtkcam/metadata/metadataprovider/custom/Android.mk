...
#-----------------------------------------------------------
# version
MY_CUST_VERSION := $(shell date) rev:0.2.0

#-----------------------------------------------------------
MY_CUST_METADATA_PATH := $(LOCAL_PATH)/config
MY_CUST_METADATA_PATH += device/leeco/x3/vendor/mediatek/proprietary/custom/mt6795/hal/imgsensor

#-----------------------------------------------------------
#
define my-all-config.metadata-under
$(patsubst ./%,%, \
  $(shell find $(1)  -maxdepth 1 \( -name "v3_config_static_metadata.*.h" \) -and -not -name ".*") \
 )
endef
#
# custom feature table file list
MY_CUST_METADATA_FILE_LIST := $(call my-all-config.metadata-under, $(MY_CUST_METADATA_PATH))
...
$(MY_CUST_METADATA_FINAL_FILE): $(MY_CUST_METADATA_FILE_LIST)
	...
	@echo '#define MY_CUST_FTABLE_FINAL_FILE "$(MY_CUST_METADATA_FINAL_FILE)"' >> $@
	@for x in $(MY_CUST_METADATA_FILE_LIST); do echo "#include <`basename $$x`>" >> $@; done
