...
$(patsubst ./%,%, \
  $(shell find $(1)  -maxdepth 1 \( -name "v3_config_request_metadata.*.h" \) -and -not -name ".*") \
 )
endef
#
# custom feature table file list
MY_CUST_REQUEST_FILE_LIST := $(call my-all-config.request-under, $(MY_CUST_REQUEST_PATH))
...
$(MY_CUST_REQUEST_FINAL_FILE): $(MY_CUST_REQUEST_FILE_LIST)
	...
	@echo '#define MY_CUST_FTABLE_FINAL_FILE "$(MY_CUST_REQUEST_FINAL_FILE)"' >> $@
	@for x in $(MY_CUST_REQUEST_FILE_LIST); do echo "#include <`basename $$x`>" >> $@; done
