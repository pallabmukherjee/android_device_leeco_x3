...
static ISP_MESSAGE_ENUM dual_cal_rrz(const ISP_DUAL_IN_CONFIG_STRUCT *ptr_in_param, ISP_DUAL_OUT_CONFIG_STRUCT *ptr_out_param)
{
		...
		ptr_in_param->CAM_CTL_EN_P1.Bits.RMX_EN && ptr_in_param->CAM_CTL_EN_P1.Bits.DMX_EN)
	{
		if (ptr_in_param->CAM_CTL_EN_P1.Bits.RRZ_EN)
		{
			...
			/* left side */
			ptr_out_param->CAM_RRZ_HORI_STEP.Bits.RRZ_HORI_STEP = ((rrz_in_w - 1 - ptr_in_param->SW.TWIN_RRZ_HORI_INT_OFST -
				ptr_in_param->SW.TWIN_RRZ_HORI_INT_OFST_LAST)*DUAL_RRZ_PREC)/(ptr_in_param->SW.TWIN_RRZ_OUT_WD - 1);
			...
			else
			{
				...
				else
				{
					...
					ptr_out_param->CAM_RRZ_VERT_STEP.Bits.RRZ_VERT_STEP = ((ptr_in_param->SW.TWIN_RRZ_IN_CROP_HT - 1)*
