...
#include "pic/MFB/mc1_160x120_yuy2.h"
...
/*********************************************************************************/
void *Vss_Mfb_Bld_Thread(void *data)
{
	int ret=0;
	NSCam::NSIoPipe::NSPostProc::IFeatureStream* pMFBMixStream;
    #if 0
	pMFBMixStream= NSCam::NSIoPipe::NSPostProc::IFeatureStream::createInstance(LOG_TAG, NSCam::NSIoPipe::NSPostProc::EFeatureStreamTag_MFB_Bld,0xFFFF);
    #else
	pMFBMixStream= NSCam::NSIoPipe::NSPostProc::IFeatureStream::createInstance(LOG_TAG, NSCam::NSIoPipe::NSPostProc::EFeatureStreamTag_MFB_Bld_Vss,0xFFFF);
    #endif
	pMFBMixStream->init();
	IMemDrv* mpImemDrv=NULL;
	mpImemDrv=IMemDrv::createInstance();
	...
	MINT32 bufBoundaryInBytes[3] = {0, 0, 0};
	IImageBuffer* srcBuffer;
	MUINT32 bufStridesInBytes[3] = {640, 0, 0};
    imgiBuf.size=sizeof(B1_320x240_yuy2);
	mpImemDrv->allocVirtBuf(&imgiBuf);
    memcpy( (MUINT8*)(imgiBuf.virtAddr), (MUINT8*)(B1_320x240_yuy2), imgiBuf.size);
    //imem buffer 2 image heap
    PortBufInfo_v1 portBufInfo = PortBufInfo_v1( imgiBuf.memID,imgiBuf.virtAddr,0,imgiBuf.bufSecu, imgiBuf.bufCohe);
    IImageBufferAllocator::ImgParam imgParam = IImageBufferAllocator::ImgParam((eImgFmt_YUY2),MSize(320, 240), bufStridesInBytes, bufBoundaryInBytes, 1);
    sp<ImageBufferHeap> pHeap = ImageBufferHeap::create( LOG_TAG, imgParam,portBufInfo,true);
	srcBuffer = pHeap->createImageBuffer();
    srcBuffer->incStrong(srcBuffer);
    srcBuffer->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
	...
	IMEM_BUF_INFO vipiBuf;
	IImageBuffer* vipi_srcBuffer;
	MUINT32 bufVipiStridesInBytes[3] = {320, 0, 0};
   	vipiBuf.size=sizeof(mc1_160x120_yuy2);
	mpImemDrv->allocVirtBuf(&vipiBuf);
    memcpy( (MUINT8*)(vipiBuf.virtAddr), (MUINT8*)(mc1_160x120_yuy2), vipiBuf.size);
    //imem buffer 2 image heap
    PortBufInfo_v1 vipi_portBufInfo = PortBufInfo_v1( vipiBuf.memID,vipiBuf.virtAddr,0,vipiBuf.bufSecu, vipiBuf.bufCohe);
    IImageBufferAllocator::ImgParam vipi_imgParam = IImageBufferAllocator::ImgParam((eImgFmt_YUY2),MSize(160, 120), bufVipiStridesInBytes, bufBoundaryInBytes, 1);
    sp<ImageBufferHeap> vipi_pHeap = ImageBufferHeap::create( LOG_TAG, vipi_imgParam,vipi_portBufInfo,true);
    vipi_srcBuffer = vipi_pHeap->createImageBuffer();
    vipi_srcBuffer->incStrong(vipi_srcBuffer);
    vipi_srcBuffer->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
	...
	for(int i=0;i<1;i++)
	{
		...
		crop.mCropRect.p_integral.x=0;
		crop.mCropRect.p_integral.y=0;
		crop.mCropRect.s.w=320;
		crop.mCropRect.s.h=240;
		crop.mResizeDst.w=320;
		crop.mResizeDst.h=240;
		int wdma_out_w=0,wdma_out_h=0;
		int wrot_out_w=0,wrot_out_h=0;
		...
		crop2.mCropRect.p_integral.x=0;
		crop2.mCropRect.p_integral.y=0;
		crop2.mCropRect.s.w=320;
		crop2.mCropRect.s.h=240;
		crop2.mResizeDst.w=320;
		crop2.mResizeDst.h=240;
		wdma_out_w=320;
		wdma_out_h=240;
		wrot_out_w=320;
		wrot_out_h=240;
		...
		//dump image
		char filename[256];
		sprintf(filename, "/sdcard/Pictures/P2VSS_MFBBld_%d_img3o_%dx%d.yuv",i, 5280,960);
		saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), 5280 *960 * 2);
    	sprintf(filename, "/sdcard/Pictures/P2VSS_MFBBld_%d_wdmao_%dx%d.yuv",i, wdma_out_w,wdma_out_h);
    	saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[1].mBuffer->getBufVA(0)), wdma_out_w *wdma_out_h * 2);
    	sprintf(filename, "/sdcard/Pictures/P2VSS_MFBBld_%d_wroto_%dx%d.yuv",i, wrot_out_w,wrot_out_h);
    	saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[2].mBuffer->getBufVA(0)), wrot_out_w *wrot_out_h * 2);
...
void *Vss_Mfb_Mix_Thread(void *data)
{
	int ret=0;
	NSCam::NSIoPipe::NSPostProc::IFeatureStream* pMFBMixStream;

	pMFBMixStream= NSCam::NSIoPipe::NSPostProc::IFeatureStream::createInstance(LOG_TAG, NSCam::NSIoPipe::NSPostProc::EFeatureStreamTag_MFB_Mix_Vss,0xFFFF);
	...
	for(int i=0;i<3;i++)
	{
		...
		crop2.mCropRect.s.w=5280;
		crop2.mCropRect.s.h=960;
		crop2.mResizeDst.w=5280;
		crop2.mResizeDst.h=960;
		wdma_out_w=1280;
		wdma_out_h=720;
		wrot_out_w=1920;
		wrot_out_h=1080;
		...
		//dump image
		char filename[256];
		sprintf(filename, "/sdcard/Pictures/P2VSS_MFBMix_%d_img3o_%dx%d.yuv",i, 5280,960);
		saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), 5280 *960 * 2);
    	sprintf(filename, "/sdcard/Pictures/P2VSS_MFBMix_%d_wdmao_%dx%d.yuv",i, wdma_out_w,wdma_out_h);
    	saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[1].mBuffer->getBufVA(0)), wdma_out_w *wdma_out_h * 2);
    	sprintf(filename, "/sdcard/Pictures/P2VSS_MFBMix_%d_wroto_%dx%d.yuv",i, wrot_out_w,wrot_out_h);
    	saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[2].mBuffer->getBufVA(0)), wrot_out_w *wrot_out_h * 2);
...
void *Vss_Mfb_Cap_Thread(void *data)
{
	int ret=0;
    //
	NSCam::NSIoPipe::NSPostProc::IFeatureStream* pP2MfbCapVss;
	pP2MfbCapVss= NSCam::NSIoPipe::NSPostProc::IFeatureStream::createInstance(LOG_TAG, NSCam::NSIoPipe::NSPostProc::EFeatureStreamTag_MFB_Cap_Vss,0xFFFF);
	pP2MfbCapVss->init();
    //
	IMemDrv* mpImemDrv=NULL;
	mpImemDrv=IMemDrv::createInstance();
	...
	//
	//
	//vss flow in featurestream
	MINT32 bufBoundaryInBytes[3] = {0, 0, 0};
	QParams p2VssEenqueParams;
	p2VssEenqueParams.mvPrivaData.push_back(NULL);
	p2VssEenqueParams.mvMagicNo.push_back(0);
	//input image
	IMEM_BUF_INFO imgiBuf;
	MINT32 bufVssBoundaryInBytes[3] = {0, 0, 0};
	IImageBuffer* srcVssBuffer;
	MUINT32 bufVssStridesInBytes[3] = {3200, 0, 0};
    imgiBuf.size=sizeof(g_imgi_array_2560x1440_b10);
	mpImemDrv->allocVirtBuf(&imgiBuf);
    memcpy( (MUINT8*)(imgiBuf.virtAddr), (MUINT8*)(g_imgi_array_2560x1440_b10), imgiBuf.size);
     //imem buffer 2 image heap
    PortBufInfo_v1 portVssBufInfo = PortBufInfo_v1( imgiBuf.memID,imgiBuf.virtAddr,0,imgiBuf.bufSecu, imgiBuf.bufCohe);
    IImageBufferAllocator::ImgParam imgVssParam = IImageBufferAllocator::ImgParam((eImgFmt_BAYER10),MSize(2560, 1440), bufVssStridesInBytes, bufVssBoundaryInBytes, 1);
    sp<ImageBufferHeap> pVssHeap = ImageBufferHeap::create( LOG_TAG, imgVssParam,portVssBufInfo,true);
	srcVssBuffer = pVssHeap->createImageBuffer();
    srcVssBuffer->incStrong(srcVssBuffer);
    srcVssBuffer->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
	Input srcVss;
	srcVss.mPortID=IMGI;
	srcVss.mBuffer=srcVssBuffer;
	p2VssEenqueParams.mvIn.push_back(srcVss);
	//crop information
	MCrpRsInfo vssCrop;
	vssCrop.mGroupID=1;
	MCrpRsInfo vssCrop2;
	...
	{
		//clear
		p2VssEenqueParams.mvOut.clear();
		p2VssEenqueParams.mvCropRsInfo.clear();
		////////////////////////////////////////////////////////
		//p2Vss_Cap
		////////////////////////////////////////////////////////
		//vssCrop
		vssCrop.mCropRect.p_fractional.x=0;
		vssCrop.mCropRect.p_fractional.y=0;
		vssCrop.mCropRect.p_integral.x=0;
		vssCrop.mCropRect.p_integral.y=0;
		vssCrop.mCropRect.s.w=2560;
		vssCrop.mCropRect.s.h=1440;
		vssCrop.mResizeDst.w=2560;
		vssCrop.mResizeDst.h=1440;
		int wdma_out_w=0,wdma_out_h=0;
		int wrot_out_w=0,wrot_out_h=0;
        //
		vssCrop2.mCropRect.p_fractional.x=0;
		vssCrop2.mCropRect.p_fractional.y=0;
		vssCrop2.mCropRect.p_integral.x=0;
		vssCrop2.mCropRect.p_integral.y=0;
		vssCrop2.mCropRect.s.w=2560;
		vssCrop2.mCropRect.s.h=1440;
		vssCrop2.mResizeDst.w=2560;
		vssCrop2.mResizeDst.h=1440;
        //
		wdma_out_w=1920;
		wdma_out_h=1080;
		wrot_out_w=1280;
		...
		p2VssEenqueParams.mvCropRsInfo.push_back(vssCrop);
		p2VssEenqueParams.mvCropRsInfo.push_back(vssCrop2);
		//
		IMEM_BUF_INFO wdmaoBuf;
		IMEM_BUF_INFO wrotoBuf;
		wdmaoBuf.size=wdma_out_w*wdma_out_h*2;
		...
		PortBufInfo_v1 portBufInfo_3 = PortBufInfo_v1( wdmaoBuf.memID,wdmaoBuf.virtAddr,0,wdmaoBuf.bufSecu, wdmaoBuf.bufCohe);
	        IImageBufferAllocator::ImgParam imgParam_3 = IImageBufferAllocator::ImgParam((eImgFmt_YV12),
	                                                        MSize(wdma_out_w, wdma_out_h),  bufStridesInBytes_3, bufBoundaryInBytes, 3);
		sp<ImageBufferHeap> pHeap_3 = ImageBufferHeap::create( LOG_TAG, imgParam_3,portBufInfo_3,true);
		IImageBuffer* outBuffer1 = pHeap_3->createImageBuffer();
	        outBuffer1->incStrong(outBuffer1);
	        outBuffer1->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
		Output dst1;
		dst1.mPortID=WDMAO;
		dst1.mBuffer=outBuffer1;
		dst1.mPortID.group=0;
		p2VssEenqueParams.mvOut.push_back(dst1);
		//
		wrotoBuf.size=wrot_out_w*wrot_out_h*2;
        mpImemDrv->allocVirtBuf(&wrotoBuf);
		memset((MUINT8*)wrotoBuf.virtAddr, 0xffffffff, wrotoBuf.size);
		...
		PortBufInfo_v1 portBufInfo_4 = PortBufInfo_v1( wrotoBuf.memID,wrotoBuf.virtAddr,0,wrotoBuf.bufSecu, wrotoBuf.bufCohe);
	        IImageBufferAllocator::ImgParam imgParam_4 = IImageBufferAllocator::ImgParam((eImgFmt_YUY2),
	                                                        MSize(wrot_out_w, wrot_out_h),  bufStridesInBytes_4, bufBoundaryInBytes, 1);
		sp<ImageBufferHeap> pHeap_4 = ImageBufferHeap::create( LOG_TAG, imgParam_4,portBufInfo_4,true);
		IImageBuffer* outBuffer2 = pHeap_4->createImageBuffer();
        outBuffer2->incStrong(outBuffer2);
        outBuffer2->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
		Output dst2;
		dst2.mPortID=WROTO;
		dst2.mBuffer=outBuffer2;
		dst2.mPortID.group=0;
		p2VssEenqueParams.mvOut.push_back(dst2);
        //
		printf("(%d) : p2VssCap enque\n",i);
		ret=pP2MfbCapVss->enque(p2VssEenqueParams);
		if(!ret){
			printf("(%d) : p2Vss ERR enque fail\n",i);
		}else{
			printf("(%d) : p2Vss enque done\n",i);
		}
		QParams p2adequeParams;
		ret=pP2MfbCapVss->deque(p2adequeParams);
		if(!ret){
			printf("(%d) : p2Vss ERR deque fail\n",i);
		}else{
			printf("(%d) : p2Vss deque done\n",i);
		}
		printf("(%d) : (CAM_CTL_TILE  0x%x) \n",i,ISP_READ_REG_NOPROTECT(mpIspDrv,CAM_CTL_TILE));
		//dump image
		char filename[256];
        sprintf(filename, "/sdcard/Pictures/P2UT_VSS_Cap_%d_wdmao_%dx%d.yuv",i, wdma_out_w,wdma_out_h);
        saveBufToFile(filename, reinterpret_cast<MUINT8*>(p2adequeParams.mvOut[0].mBuffer->getBufVA(0)), wdma_out_w *wdma_out_h * 2);
        sprintf(filename, "/sdcard/Pictures/P2UT_VSS_Cap_%d_wroto_%dx%d.yuv",i, wrot_out_w,wrot_out_h);
        saveBufToFile(filename, reinterpret_cast<MUINT8*>(p2adequeParams.mvOut[1].mBuffer->getBufVA(0)), wrot_out_w *wrot_out_h * 2);
		printf("(%d) : --- p2Vss_Cap done ---\n", i);

        //
        //unmap and free buffer
        //p2Vss
        outBuffer1->unlockBuf(LOG_TAG);
        outBuffer2->unlockBuf(LOG_TAG);
        mpImemDrv->freeVirtBuf(&wdmaoBuf);
        mpImemDrv->freeVirtBuf(&wrotoBuf);
	}
	//
	pP2MfbCapVss->uninit();
	pP2MfbCapVss->destroyInstance(LOG_TAG);
...
int case3() // for ZSD+MFLL (driver interface of camera1.0)
{
	int ret=0;
	//MBOOL isV3 = MFALSE;	//temp disable tuning path cuz tuning function not ready
	NSCam::NSIoPipe::NSPostProc::IFeatureStream* pP2AStream;
	pP2AStream= NSCam::NSIoPipe::NSPostProc::IFeatureStream::createInstance(LOG_TAG, NSCam::NSIoPipe::NSPostProc::EFeatureStreamTag_vFB_Stream,0xFFFF);
	pP2AStream->init();
    //
	IMemDrv* mpImemDrv=NULL;
	mpImemDrv=IMemDrv::createInstance();
	...
	#if 0
	//vss flow in featurestream
	QParams p2FeatureEenqueParams;
	p2FeatureEenqueParams.mvPrivaData.push_back(NULL);
	p2FeatureEenqueParams.mvMagicNo.push_back(0);
	//input image
	//IMEM_BUF_INFO imgiBuf;
	MINT32 bufVssBoundaryInBytes[3] = {0, 0, 0};
	IImageBuffer* srcVssBuffer;
	...
	srcVss.mPortID=IMGI;
	srcVss.mBuffer=srcVssBuffer;
	p2FeatureEenqueParams.mvIn.push_back(srcVss);
	//crop information
	MCrpRsInfo vssCrop;
	vssCrop.mGroupID=1;
	MCrpRsInfo VssCrop2;
    ...
    #if 0  // for MFB VSS Capture
    threadId = pthread_create(&pThread[0], NULL, Vss_Mfb_Cap_Thread, (void *)&threadData);
    #endif
    #if 1  // for MFB VSS blending
    threadId = pthread_create(&pThread[0], NULL, Vss_Mfb_Bld_Thread, (void *)&threadData);
    #endif
    #if 0  // for MFB VSS mixing
    threadId = pthread_create(&pThread[0], NULL, Vss_Mfb_Mix_Thread, (void *)&threadData);
    #endif
    ...
    if (threadId < 0)
    {
        printf("[Error]thread create error(%d)",threadId);
        return -1;
    }
	//output buffer
	for(int i=0;i<3;i++)
	{
		...
		dst1.mBuffer=outBuffer1;
		dst1.mPortID.group=0;
		p2aenqueParams.mvOut.push_back(dst1);
		//
		wrotoBuf.size=wrot_out_w*wrot_out_h*2;
        mpImemDrv->allocVirtBuf(&wrotoBuf);
		memset((MUINT8*)wrotoBuf.virtAddr, 0xffffffff, wrotoBuf.size);
		...
		//dump image
		char filename[256];
        sprintf(filename, "/sdcard/Pictures/P2UT_p2a_%d_wdmao_%dx%d.yuv",i, wdma_out_w,wdma_out_h);
        saveBufToFile(filename, reinterpret_cast<MUINT8*>(p2adequeParams.mvOut[0].mBuffer->getBufVA(0)), wdma_out_w *wdma_out_h * 2);

        sprintf(filename, "/sdcard/Pictures/P2UT_p2a_%d_wroto_%dx%d.yuv",i, wrot_out_w,wrot_out_h);
        saveBufToFile(filename, reinterpret_cast<MUINT8*>(p2adequeParams.mvOut[1].mBuffer->getBufVA(0)), wrot_out_w *wrot_out_h * 2);
