        ...
        if ((i4ActiveItem & E_AE_AE_CALC) || (bCalculateAE == MTRUE)) {
            ...
            if(m_eZoomWinInfo.bZoomChange == MTRUE) {
                ...
                if (m_bPerframeAEFlag){
                    ...
                    MY_LOG("[%s():MgrToAlgo] rAEInput.PreEvSetting[N-1]:%d/%d/%d rAEInput.PreEvSetting[N-2]:%d/%d/%d rAEInput.PreEvSetting[N-3]:%d/%d/%d rAEInput.CycleInfo.m_i4FrameCnt:%d\n", __FUNCTION__, 
                ...
                if(m_pIAeAlgo != NULL) {
                    if(m_bRestoreAE == MFALSE) {
                        ...
                        MY_LOG("[%s():AlgoToMgr] Exp/Afe/Isp= %d/%d/%d \n", __FUNCTION__, rAEOutput.EvSetting.u4Eposuretime,rAEOutput.EvSetting.u4AfeGain,rAEOutput.EvSetting.u4IspGain);
...
if(m_bPerframeAEFlag){
            ...
            if ((m_i4SensorGainDelayFrames == m_i4ShutterDelayFrames)||m_rAEInitInput.rAEPARAM.bPerFrameAESmooth) {
                    ...
                    m_u4UpdateGainValue = m_rAEOutput.rPreviewMode.u4AfeGain;
            } else if (m_i4SensorGainDelayFrames == m_i4ShutterDelayFrames + 1){
                    m_bSetGainValue = MTRUE;
                    m_u4UpdateGainValue = m_u4PreviousSensorgain;
            } else{
                    MY_LOG("Perframe update AE warning : AfeGain delay should be more than 1.\n");
            }
            // Calculate Real ISO
            if(m_p3ANVRAM != NULL) {
                    rAEInfo2ISP.u4RealISOValue = (m_p3ANVRAM->rAENVRAM.rDevicesInfo.u4MiniISOGain*((rAEInfo2ISP.u4AfeGain*rAEInfo2ISP.u4IspGain)>>10))>>10;
                ...
                case 6: // Perframe command IT [10ms/3x<->30ms/1x]
                    if(m_u4IndexTestCnt%2 == 0) {
                        ...
                        if(m_i4ShutterDelayFrames == m_i4SensorGainDelayFrames) {
                            m_rAEOutput.rPreviewMode.u4AfeGain = 1024*3;
    	                } else {
                            m_rAEOutput.rPreviewMode.u4AfeGain = 1024;
                        }
...
MRESULT AeMgr::monitorAndReschedule( MBOOL bAAASchedule, MUINT32 u4AvgYcur, MUINT32 u4AvgYStable, MBOOL *bAECalc, MBOOL *bAEApply )
{
    ...
    MUINT8 NumCycleAESkipAfterExit = 1;
