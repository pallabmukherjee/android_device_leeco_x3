...
doEis(MUINT32 ref, MINT32 &gmvX, MINT32 &gmvY) 
{
	...
	//for(int i = 1; i < FrameNum; i++)	//for each frame
	{
		...
		if( cnt_x_tmp == 0 )
			...
			sum_x_tmp = sum_x_tmp  / cnt_x_tmp;

		if( cnt_y_tmp == 0 )
			sum_y_tmp = 0;
		else
			sum_y_tmp = sum_y_tmp  / cnt_y_tmp;

		mAccGmvX += (MFLOAT)sum_x_tmp* CUST_MFLL_EIS_OP_STEP_H/256;
		mAccGmvY += (MFLOAT)sum_y_tmp* CUST_MFLL_EIS_OP_STEP_V/256;
