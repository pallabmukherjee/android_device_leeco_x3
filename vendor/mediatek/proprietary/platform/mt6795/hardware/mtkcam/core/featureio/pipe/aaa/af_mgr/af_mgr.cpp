...
MRESULT AfMgr::init(MINT32 i4SensorIdx, MINT32 isInitMCU)
{
	...
	// check support PDAF or not
	  	...
	  	AAASensorMgr::getInstance().getSensorVCinfo(m_i4CurrSensorDev, &SensorVCInfo);
...
MRESULT AfMgr::setAFArea(CameraFocusArea_T a_sAFArea)
{
    ...
    {
         MY_LOG("[setAFArea] @LIB3A_AF_MODE_AFC\n");
    ...
    {
        MY_LOG("[setAFArea] u4Count==0@tcaf mode, return\n");
    ...
    if ((a_sAFArea.u4Count == 0) || (a_sAFArea.rAreas[0].i4Left == a_sAFArea.rAreas[0].i4Right) || (a_sAFArea.rAreas[0].i4Top == a_sAFArea.rAreas[0].i4Bottom))
    {
        ...
        MY_LOG("[setAFArea] u4Count==0, percent_W %d, percent_H %d\n", m_NVRAM_LENS.rAFNVRAM.i4SPOT_PERCENT_W, m_NVRAM_LENS.rAFNVRAM.i4SPOT_PERCENT_H);
    ...
    if (m_sAFInput.sAFArea.i4Count != 0)
    {
        ...
        {
	    MY_LOG("[setAFArea] @Zoom_W %d, Zoom_H %d, Zoom_x %d, Zoom_Y %d\n", m_sAFInput.sEZoom.i4W, m_sAFInput.sEZoom.i4H, m_sAFInput.sEZoom.i4X, m_sAFInput.sEZoom.i4Y);
            MY_LOG("[setAFArea] @Cnt %d, [L]%d, [R]%d, [T]%d, [B]%d\n", i, a_sAFArea.rAreas[i].i4Left, a_sAFArea.rAreas[i].i4Right, a_sAFArea.rAreas[i].i4Top, a_sAFArea.rAreas[i].i4Bottom);
...
MVOID AfMgr::setFlkWinConfig()
{
    ...
    if(m_eSensorTG ==ESensorTG_1)
    {
        ...
        else
        {
            ...
            wintmp=(Boundary(WIN_POS_MIN, i4Y + i4H*4, i4HOri-WIN_POS_MIN)&0x1FFE)+ ((Boundary(WIN_POS_MIN, i4Y + i4H*5 - m_i4AF_in_Vsize/10, i4HOri-WIN_POS_MIN)&0x1FFE)<<16);
...
AF_STAT_T AfMgr::Trans4WintoOneStat(MVOID *pAFStatBuf)
{
    ...
    for (MINT32 i=0; i<(MAX_AF_HW_WIN-1); i++)
    {
        ...
        statsH[i] = ((((MINT64)pAFStat->sStat[i].u4StatV>>28)&0xF)<<32) + (MINT64)pAFStat->sStat[i].u4Stat24;;
        statsV[i] = pAFStat->sStat[i].u4StatV&0x3FFFFFF;
    }
    // --- Floating ---
        ...
        MY_LOG("FL_FV %lld %lld", statsH[MAX_AF_HW_WIN-1], statsV[MAX_AF_HW_WIN-1]);
    if (m_pIAfAlgo)
    {
...
//  add by yfx
	...
	MY_LOG("read imx230 calibration data from EEPROM !!");
	memset(t_buffer,0xFF,t_size);
	char*ptr=t_buffer;
	pIHalSensor->sendCommand(m_i4CurrSensorDev,SENSOR_CMD_GET_PDAF_DATA, (MUINTPTR)&t_offset, (MUINTPTR)&ptr, (MUINTPTR)&t_size);
	MINT32 i;
	for(i = 0; i < 96; i++)
	{
		MY_LOG("[DCC][0x%x] = 0x%x", i, t_buffer[i]);
	}	
	memcpy(m_DccData, t_buffer,t_size); 
...
MVOID AfMgr::VsyncSignal()
{
	...
	if( m_AAOPDBuf)
	{
		MBOOL ret;
		// Dequeue DMA buffer
		ret = IAEBufMgr::getInstance().dequeueHwBuf( m_i4CurrSensorDev, rBufInfo, _PD_BUF_SEL_);
		if( ret==MTRUE)
		{
		//allocate pd input data
