...
MAP_TYPE2BIT(SHOTCB_STEREO_RAWDATA_SRC   , 27);
MAP_TYPE2BIT(CONTROL_STEREO_RAW_DST   , 28);
...
)
{
    ...
    EIspProfile_T capprofile = EIspProfile_N3D_Capture;
...
connectNodes(MUINT32 NodeDataSet)
{
    ...
    // [ P1Node -- SyncNode]    
    // mpGraph->connectData(   PASS1_RESIZEDRAW,       SYNC_SRC_0,             mpPass1,                mpSyncNode);    
    // mpGraph->connectData(   PASS1_RESIZEDRAW,       SYNC_SRC_1,             mpPass1_Main2,          mpSyncNode);
    // mpGraph->connectData(   PASS1_FULLRAW,          SYNC_SRC_2,             mpPass1,                mpSyncNode);
    // // [ SyncNode -- CtrlNode]
    // mpGraph->connectData(   SYNC_DST_0,             CONTROL_RESIZEDRAW,     mpSyncNode,             mpCtrl);
    // mpGraph->connectData(   SYNC_DST_1,             CONTROL_RESIZEDRAW,     mpSyncNode,             mpCtrl_Main2);
    // mpGraph->connectData(   SYNC_DST_2,             CONTROL_FULLRAW,        mpSyncNode,             mpCtrl);
    // // [CtrlNdoe -- SNode]    
    // mpGraph->connectData(   CONTROL_PRV_SRC,        STEREO_SRC,             mpCtrl,                 mpStereoNode);
    // mpGraph->connectData(   CONTROL_PRV_SRC,        STEREO_SRC,             mpCtrl_Main2,           mpStereoNode_Main2);
    // mpGraph->connectData(   CONTROL_CAP_SRC,        PASS2_CAP_SRC,          mpCtrl,                 mpStereoCapNode);
    ...
    mpGraph->connectData(   CONTROL_STEREO_RAW_DST,        SHOTCB_RAWDATA_SRC,   mpCtrl,           mpCallback);
