...
int CamPathPass2::getTpipePerform( struct CamPathPass2Parameter* p_parameter )
{
    ...
    //due to cmdq trigger isp as many as the tpipe numbers, we have to always set irq mode to notify tpipe driver stop continuing calcualtion next tpipe
    ...
    if( p_parameter->isp_top_ctl.CAM_CTL_EN_P2_DMA.Raw & (CAM_CTL_EN_P2_DMA_MDP_WROTO_EN|CAM_CTL_EN_P2_DMA_MDP_WDMAO_EN|CAM_CTL_EN_P2_DMA_MDP_JPEGO_EN|CAM_CTL_EN_P2_DMA_MDP_VENC_EN)){
    	...
    	if(p_parameter->isp_top_ctl.CAM_CTL_EN_P2.Bits.MFB_EN)
    	{	//due to cmdq trigger isp as many as the tpipe numbers, we have to set irq mode to notify tpipe driver stop continuing calcualtion next tpipe
    		// after each one is finished
    		tdriPipe.tdri.tdriPerformCfg.irqMode = TPIPE_IRQ_TPIPE;
    	}
    	else
...
int CamPathPass2::configTpipeData( struct CamPathPass2Parameter* p_parameter )
{
    ...
    //
    MUINT8 block_size;
    //tdriPipe.tdri.fe.fe_mode = 2; /* fe_mode */  //kk test
    //tdriPipe.tdri.fe.fe_mode = 1; /* fe_mode */  //kk test
    tdriPipe.tdri.fe.fe_mode=p_parameter->fe.fe_mode;
    switch (tdriPipe.tdri.fe.fe_mode)
    {
