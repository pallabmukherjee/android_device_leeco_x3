...
MINT32 u4PortID[DMA_PORT_TYPE_NUM][DMA_OUT_PORT_NUM] = {/*0*/{(MINT32)EPortIndex_IMG2O, (MINT32)EPortIndex_IMG3O, (MINT32)EPortIndex_WDMAO, (MINT32)EPortIndex_WROTO, (MINT32)EPortIndex_JPEGO, (MINT32)EPortIndex_FEO, (MINT32)EPortIndex_VIPI},
 ...
 Scen_Map_CropPathInfo_STRUCT mCropPathInfo[]
    ...
    {eDrvScenario_IP_TPipe,        (MUINT32)(ECropGroupIndex_NONE),                &(u4PortID[6][0])},
...
//    }
	...
	//fe block size
	//stereo cam would not refer the image w/h to set block size for fe while eis would
	//we add a customized parameter of fe_block_size for stereo cam
	//and adopt w/h judgement for block size with eis
	int fe_block_size=-1;
    ...
    for (MUINT32 i = 0 ; i < vInPorts.size() ; i++ )
	{
        ...
        switch(vInPorts[i]->index)
        {
			case EPortIndex_IMGI:
                ...
                // check line buffer for ip raw
                //if((meScenarioID==eDrvScenario_IP)&&(portInfo_imgi.u4ImgWidth > CAM_CTL_BNR_LINE_BUFFER)){
                //    PIPE_ERR("BNR support max line buffer is 5312,imgi_w(%d)",portInfo_imgi.u4ImgWidth);
                //    return MFALSE;
                //}
                ...
                PIPE_INF("[Imgi][crop_1](x,f_x,y,f_y)=(%d,%d,%d,%d),(w,h,tarW,tarH)=(%d,%d,%d,%d)--[crop_2](x,f_x,y,f_y)=(%d,%d,%d,%d),(w,h,tarW,tarH)=(%d,%d,%d,%d),pass2_CQ(%d),dupCqIdx(%d)", \
                        ...
                        portInfo_imgi.crop2.w,portInfo_imgi.crop2.h,portInfo_imgi.resize2.tar_w,portInfo_imgi.resize2.tar_h,pass2_CQ,dupCqIdx);
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////
    //calculate crop offset of img3o for tpipe
    //align crz if enable img2o+img3o when nr3d is before crz
    //align mdp crop if enable mdp output+imf3o
    //align mdp crop if enable img2o+mdp output+img3o
    if(idx_img3o >= 0)
    {
    		//enable img2o when nr3d is before crz
    		if(idx_img2o >=0)
    		{
    			switch(meScenarioID)
    			{
                    ...
                    portInfo_img3o.xoffset = portInfo_imgi.crop1.x & (~1);	//img3o offset must be even
			  		...
			  		PIPE_INF("IMG3O cpoft_crz(%d/%d)",portInfo_img3o.xoffset,portInfo_img3o.yoffset);
    					break;
    				default:
    					break;
    			}
	  	...
	  	{	//would enable mdp output
	  		...
	  		portInfo_img3co.xoffset = portInfo_img3o.xoffset;
	  		portInfo_img3co.yoffset = portInfo_img3o.yoffset;
	  		PIPE_INF("IMG3O cpoft_mdp(%d/%d)",portInfo_img3o.xoffset,portInfo_img3o.yoffset);
  		}
  	}
    /////////////////////////////////////////////////////////////////////////////////////////////
	//
	for (MUINT32 i = 0 ; i < vModuleParams.size() ; i++ )
	{
        switch(vModuleParams[i].eP2module){
            ...
            case EP2Module_SRZ1:
            	...
            	break;
            case EP2Module_FE:
            	fe_block_size=reinterpret_cast<FECfg*>(vModuleParams[i].moduleStruct)->block_size;
            	PIPE_INF("fe blk sz(%d)",fe_block_size);
            	break;
            default:
                PIPE_ERR("[Error]do not support this vOutPorts(%d),i(%d)",vOutPorts[i]->index,i);
                break;
...
//                en_p2_dma |=  CAM_CTL_DMA_EN_VIPI_EN;
    ...
    if (-1 != idx_feo )
    {
        	...
        	m_camPass2Param.fe.fe_mode = 1;
		//fe mode criterion
		MINT32 p2OUT_w=0;
		MINT32 p2OUT_h=0;

		if(fe_block_size != -1)
		{	//customized paramter for stereo cam
			PIPE_INF("[FEBLOCKSET]fe_block_size (%d)",fe_block_size);
			switch(fe_block_size)
			{
				case 8:
					m_camPass2Param.fe.fe_mode = 2;
					break;
				case 16:
					m_camPass2Param.fe.fe_mode = 1;
					break;
				case 32:
					m_camPass2Param.fe.fe_mode = 0;
					break;
				default:
					PIPE_ERR("wrong block size(%d)",fe_block_size);
					break;
			}
		}
		else
		{	//use w/h judgement for eis
			if (MTRUE == m_CrzEn)
			{
				...
				{
					m_camPass2Param.fe.fe_mode = 0;
					PIPE_WRN("fe_mode should not be 0");
				}
			}
		}
		...
		switch (m_camPass2Param.fe.fe_mode)
		{
			case 0:
		...
		m_camPass2Param.feo.size.h = m_camPass2Param.feo.size.h>>(5-m_camPass2Param.fe.fe_mode);
