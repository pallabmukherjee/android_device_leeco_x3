   ...
   m_rIspCamInfo.rAWBInfo = rAWBInfo;
         ...
         bAWBGainChanged)
    {
        if (m_rIspParam.bInvokeSmoothCCM) { // smooth CCM 
            ...
            MY_LOG_IF(m_bDebugEnable, "is_to_invoke_smooth_ccm_with_preference_gain = %d, m_i4FlashOnOff = %d"
                , m_pIspTuningCustom->is_to_invoke_smooth_ccm_with_preference_gain(m_rIspCamInfo)
                , m_i4FlashOnOff);
            m_pCcmMgr->calculateCCM(rAWBInfo, m_pIspTuningCustom->is_to_invoke_smooth_ccm_with_preference_gain(m_rIspCamInfo), m_i4FlashOnOff);
        }
        else { // dynamic CCM
...
#define TO_INT(a, bit) ((MINT32)((MINT32) (a)) << (32-(bit)) >> (32-(bit)))
    ...
    in_CCM[0][0] = (double) TO_INT(ccm.cnv_1.bits.G2G_CNV_00, 13) / 512;
    in_CCM[0][1] = (double) TO_INT(ccm.cnv_1.bits.G2G_CNV_01, 13) / 512;
    in_CCM[0][2] = (double) TO_INT(ccm.cnv_2.bits.G2G_CNV_02, 13) / 512;
    in_CCM[1][0] = (double) TO_INT(ccm.cnv_3.bits.G2G_CNV_10, 13) / 512;
    in_CCM[1][1] = (double) TO_INT(ccm.cnv_3.bits.G2G_CNV_11, 13) / 512;
    in_CCM[1][2] = (double) TO_INT(ccm.cnv_4.bits.G2G_CNV_12, 13) / 512;
    in_CCM[2][0] = (double) TO_INT(ccm.cnv_5.bits.G2G_CNV_20, 13) / 512;
    in_CCM[2][1] = (double) TO_INT(ccm.cnv_5.bits.G2G_CNV_21, 13) / 512;
    in_CCM[2][2] = (double) TO_INT(ccm.cnv_6.bits.G2G_CNV_22, 13) / 512;
    
    if (CcmMgr::gen_CM_FM(in_CCM, fColorMatrix1, fForwardMatrix1))
    {
        MY_LOG("CCM: [%f, %f, %f][%f, %f, %f][%f, %f, %f]",
            in_CCM[0][0], in_CCM[0][1], in_CCM[0][2],
            in_CCM[1][0], in_CCM[1][1], in_CCM[1][2],
            in_CCM[2][0], in_CCM[2][1], in_CCM[2][2]);
        MY_LOG("CM: [%f, %f, %f][%f, %f, %f][%f, %f, %f]",
