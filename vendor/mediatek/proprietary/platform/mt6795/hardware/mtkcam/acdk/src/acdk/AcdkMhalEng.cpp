...
static MINT32 gPreW = 1280;
static MINT32 gPreH = 720;
static MINT32 gCapW = 800;
static MINT32 gCapH = 600;
...
static void handleMtkExtDataCompressedImage(const sp<IMemory>& dataPtr)
{
    ...
    g_acdkMhalCapObserver.notify(ACDK_CB_CAPTURE,(MUINT8 *)va, NULL, (MUINT32)imageSize,0);
    //
                      ...
                      void *user)
{
    ...
    switch  (msg_type & ~CAMERA_MSG_PREVIEW_METADATA)
    {
        case CAMERA_MSG_PREVIEW_FRAME:
             ...
             g_acdkMhalObserver.notify(ACDK_CB_PREVIEW,(MUINT8 *)va,NULL,(MUINT32)size,0,gPreW,gPreH);
            break;
        case CAMERA_MSG_POSTVIEW_FRAME:
             //Callback to upper layer
              ACDK_LOGD("CAMERA_MSG_POSTVIEW_FRAME=%d",CAMERA_MSG_POSTVIEW_FRAME);
             g_acdkMhalObserver.notify(ACDK_CB_PREVIEW,(MUINT8 *)va,NULL,(MUINT32)size,0,gCapW,gCapH);
            break;
...
MINT32 AcdkMhalEng::acdkMhalPreviewStart(MVOID *a_pBuffIn)
{
    ...
    //preview suppurt list 
    ACDK_LOGD("preview-size-values = %s", map.valueFor(String8("preview-size-values")).string());
    String8 preview_size_values = map.valueFor(String8("preview-size-values"));
    //acdk support preview table 
    //320,640,800,960,1280
    //240,480,600,540, 720
    if((mAcdkMhalPrvParam.frmParam.orientation == 1) ||
      (mAcdkMhalPrvParam.frmParam.orientation == 3)) 
    {
        u32width = mAcdkMhalPrvParam.frmParam.h;
    }
    else
    {
        u32width = mAcdkMhalPrvParam.frmParam.w;

    }
    u32PreviewSizeSupport = 0;
    if(u32width >= 1280)
    {
        if (preview_size_values.find("1280x720") > -1)
        {
            ACDK_LOGD("preview-size-values contains 1280x720");
            set(map, "preview-size", "1280x720");
            u32PreviewSizeSupport = 1;
            gPreW = mu4PreWidth = 1280;
            gPreH = mu4PreHeight = 720;
        }
    }
    else if(mAcdkMhalPrvParam.frmParam.w >= 960)
    {
        if (preview_size_values.find("960x540") > -1)
        {
            ACDK_LOGD("preview-size-values contains 960x540");
            set(map, "preview-size", "960x540");
            u32PreviewSizeSupport = 1;
            gPreW = mu4PreWidth = 960;
            gPreH = mu4PreHeight = 540;
        }
    }
    else
    {
        if (preview_size_values.find("800x600") > -1)
        {
            ACDK_LOGD("preview-size-values contains 800x600");
            set(map, "preview-size", "800x600");
            u32PreviewSizeSupport = 1;
            gPreW = mu4PreWidth = 800;
            gPreH = mu4PreHeight = 600;
        }
    }

    if(u32PreviewSizeSupport == 0)
    {
        ACDK_LOGE("No support preview values, Please check preview size");
    }
