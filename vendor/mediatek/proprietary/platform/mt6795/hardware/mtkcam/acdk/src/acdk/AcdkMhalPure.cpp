...
static MVOID *acdkMhalProcLoop(MVOID *arg)
{
    ...
    while(eState != ACDK_MHAL_UNINIT)
    {
        ...
        eState = g_pAcdkMHalPureObj->acdkMhalGetState();
        ACDK_LOGD("acdkMhalGetState eState %d\n",eState);
...
MINT32 AcdkMhalPure::acdkMhalPreviewStart(MVOID *a_pBuffIn)
{
    ...
    // single mode line buffer 3072 , twin mode can accept line buffer 3072 x2
    // sensor mode two pixel , isp would use twin mode ( over 13MP / 30fps)s
    m_rrzo.eImgFmt        = eImgFmt_FG_BAYER10;  //  Image Pixel Format
    m_rrzo.u4ImgWidth     = mAcdkMhalPrvParam.rrzoWidth; //3072;        //  Image Width
    ...
    if(0)//m_imgo.u4ImgWidth > 3072)
    {
        RRZCfg rrz;
        MINT32 rrzowidth,rrzoheight;

        rrzowidth = 3072;
        rrzoheight = (((m_imgo.u4ImgHeight*3072)/m_imgo.u4ImgWidth)>>2)<<2;//rrzoheight* ((MFLOAT)m_imgo.u4ImgWidth/3072);
...
MINT32 AcdkMhalPure::acdkMhalPreviewProc()
{
    ...
    else if(m_tgi.index == EPortIndex_TG2I)
    {
        ...
        //m_pICamIOPipe->irq(EPipePass_PASS1_TG2,EPIPEIRQ_VSYNC,ISP_DRV_IRQ_USER_ACDK);
    }
    ...
    while((acdkMhalGetState() & ACDK_MHAL_PREVIEW_MASK) || (ACDK_MHAL_PRE_CAPTURE == acdkMhalGetState()))
    {
        ...
        else
        {
            ...
            for(MUINT32 i = 0; i < imgo_num; ++i)
            {
                ...
                ACDK_LOGD_DYN(g_acdkMhalPureDebug,"imgo_virAddr[%d](%p),imgo_phyAddr[%d](%p)",i,imgo_virAddr[i],i,imgo_phyAddr[i]);
            }
