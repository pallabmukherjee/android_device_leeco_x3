				  ...
				  unsigned int *pMva)
{
    ...
    else
    {
        *pMva = m4u_module.MVAStart;
        M4UINFO("ioctl MTK_M4U_T_ALLOC_MVA :va = 0x%lx,size = 0x%x,mva = 0x%x,port = %d", va, size, *pMva, port);
        return 0;
    }
}
                            ...
                            unsigned int mva)
{
    ...
    {
        M4UINFO("ioctl MTK_M4U_T_DEALLOC_MVA :va = 0x%lx,size =0x%x,mva = 0x%x, port=%d", va, size, mva, port);
        if(munlock((void*)va, size)) {
            M4UINFO("munlock fail! %d, %s \n", errno, strerror(errno));
        }
        return 0;
    }
}
