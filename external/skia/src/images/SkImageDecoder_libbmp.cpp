...
SkImageDecoder::Result SkBMPImageDecoder::onDecode(SkStream* stream, SkBitmap* bm, Mode mode) {
    ...
    MtkSkDebugf("bmp_decoder onDecode stream %p, bm %p, mode %d\n", stream, bm, mode);
    ...
    MtkSkDebugf("bmp_decoder onDecode finish successfully, L:%d!!!\n",__LINE__);
...
bool SkBMPImageDecoder::onDecodeSubset(SkBitmap* bm, const SkIRect& region) {
    ...
    MtkSkDebugf("bmp_decoder onDecodeSubset bm %p, region %p\n", bm, region);
   ...
   SkColorType colorType = this->getPrefColorType(k32Bit_SrcDepth, false);
    ...
    MtkSkDebugf("bmp_decoder onDecodeSubset finish successfully, L:%d!!!\n",__LINE__);
